	...
	return (
		<AdminLayout>
			...
			<div className="w-full max-w-3xl py-10 flex flex-col flex-1 gap-6">
				<h1 className="text-2xl font-bold px-6 lg:px-0">Reservas</h1>
				{bookings.isFetching ? (
							...
							<div className="flex flex-col gap-4 w-full">
								...
								<div className="product-container">
									{bookings.data
										.filter((b) => !isAfter(new Date(b.dateStart), new Date()))
										.map((cat) => (
											<BookingItem key={cat.id} handleDelete={handleDelete} to={'/admin/booking/' + cat.id} {...cat} />
										))}
								</div>
							</div>
						)}
